    ...
    let matches = App::new("jbindgen")
        ...
        .args(&[
            ...
            Arg::with_name("jniimpl_use")
                .help(
                    "The full use path to find the JniImpl struct definition that the generated \
                    functions will expect a matching method on",
                )
                .long("jniimpl_use")
                .default_value("jniimpl::JniImpl")
                .required(true),
...
fn add_func_body(func: ForeignItemFn) -> TokenStream {
    let sig = RewriteSysTypes.fold_signature(func.sig);
...
impl Fold for RewriteSysTypes {
    fn fold_ident(&mut self, i: Ident) -> Ident {
        let name = match i.to_string().as_str() {
            "jclass" => "JClass",
            "jfieldid" => "JFieldID",
            "jmethodid" => "JMethodID",
            "jobject" => "JObject",
            "jstaticfieldid" => "JStaticFieldID",
            "jstring" => "JString",
            "jthrowable" => "JThrowable",
            "jvalue" => "JValue",
            _ => return i,
        };
        return Ident::new(name, i.span());
    }
}
